...
class TITAPointFootRoughCfg(BaseConfig):
    class env:
        num_envs = 4096
        num_propriceptive_obs = 57
    ...
    class observation:
        history_length_dof_pos = 3  # History length for joint positions
        history_length_dof_vel = 2  # History length for joint velocities
    ...
    class asset:
        ...
        if not robot_type:
            print(
                "Error: Please set the ROBOT_TYPE using 'export ROBOT_TYPE=<robot_type>'."
            )
            sys.exit(1)
        file = (
            "{LEGGED_GYM_ROOT_DIR}/resources/robots/tita_pointfoot/"
            + robot_type
            + "/urdf/tita_description.urdf"
        )
        name = robot_type
        foot_name = "_leg_4"
    ...
    class rewards:
        ...
        soft_dof_pos_limit = (
            0.8  # percentage of urdf limits, values above this limit are penalized
        )
...
class TITAPointFootRoughCfgPPO(BaseConfig):
    seed = 1
    ...
    class runner:
        ...
        experiment_name = "tita_pointfoot_rough"
        run_name = ""
        # load and resume
        resume = False
        ...
        # load_run = "/home/server/isaacgym_envs/pointfoot-legged-gym/logs/tita_pointfoot_rough/Oct23_20-30-19_"
        # checkpoint = -1  # -1 = last saved model
