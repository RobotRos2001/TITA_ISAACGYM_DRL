...
class TITA_PointFoot:
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        # allocate buffers
        self.proprioceptive_obs_buf = torch.zeros(
    ...
    def step(self, actions):
        ...
        # print("Actions of Env 0:", self.actions[0])
        ...
        self.proprioceptive_obs_buf = torch.clip(
            self.proprioceptive_obs_buf, -clip_obs, clip_obs
        )
    ...
    def post_physics_step(self):
            """Updates observations, rewards, and resets environments."""
            ...
            self.history_dof_pos = torch.roll(self.history_dof_pos, shifts=1, dims=-1)
            self.history_dof_pos[..., 0] = self.dof_pos

            self.history_dof_vel = torch.roll(self.history_dof_vel, shifts=1, dims=-1)
            self.history_dof_vel[..., 0] = self.dof_vel
            ...
            if (
                self.cfg.terrain.measure_heights_actor
                or self.cfg.terrain.measure_heights_critic
            ):
    ...
    def _check_if_include_feet_height_rewards(self):
        members = [
            attr for attr in dir(self.cfg.rewards.scales) if not attr.startswith("__")
        ]
    ...
    def _reset_buffers(self, env_ids):
        ...
        self.current_max_feet_height[env_ids] = 0.0
        self.last_max_feet_height[env_ids] = 0.0
    ...
    def _add_noise_to_obs(self):
        ...
        if self.add_noise:
            ...
            obs_noise_buf = (
                2 * torch.rand_like(self.proprioceptive_obs_buf) - 1
            ) * obs_noise_vec
            self.proprioceptive_obs_buf += obs_noise_buf
            if self.num_privileged_obs is not None:
                privileged_extra_obs_buf = (
                    2
                    * torch.rand_like(
                        self.privileged_obs_buf[:, len(self.noise_scale_vec[0]) :]
                    )
                    - 1
                ) * privileged_extra_obs_noise_vec
                self.privileged_obs_buf += torch.cat(
                    (obs_noise_buf, privileged_extra_obs_buf), dim=1
                )
    ...
    def compute_privileged_observations(self):
        if self.num_privileged_obs is not None:
            ...
            if self.cfg.terrain.measure_heights_critic:
                self.privileged_obs_buf = self._add_height_measure_to_buf(
                    self.privileged_obs_buf
                )
    ...
    def compute_proprioceptive_observations(self):
        """Computes observations and appends joint history."""
        self._compose_proprioceptive_obs_buf_no_height_measure()

        if self.cfg.terrain.measure_heights_actor:
            self.proprioceptive_obs_buf = self._add_height_measure_to_buf(
                self.proprioceptive_obs_buf
            )

        # Scale and add joint position and velocity history to the observation buffer
        scaled_history_dof_pos = (self.history_dof_pos - self.default_dof_pos.unsqueeze(-1)) * self.obs_scales.dof_pos
        scaled_history_dof_vel = self.history_dof_vel * self.obs_scales.dof_vel

        self.proprioceptive_obs_buf = torch.cat(
            (
                self.proprioceptive_obs_buf,
                scaled_history_dof_pos.reshape(self.num_envs, -1),
                scaled_history_dof_vel.reshape(self.num_envs, -1)
            ),
            dim=-1
        )
        ...
        # print("Proprioceptive Observations of Env 0:", self.proprioceptive_obs_buf[0])
    ...
    def _add_height_measure_to_buf(self, buf):
        ...
        buf = torch.cat((buf, heights), dim=-1)
        return buf
    ...
    def _get_noise_scale_vec(self):
        ...
        """
        obs_noise_vec = torch.zeros(
            self.cfg.env.num_propriceptive_obs, device=self.device
        )
        ...
        obs_noise_vec[9:command_end_idx] = 0.0  # commands
        dof_pos_end_idx = command_end_idx + self.num_dof
        obs_noise_vec[command_end_idx:dof_pos_end_idx] = (
            noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
        )
        dof_vel_end_idx = dof_pos_end_idx + self.num_dof
        obs_noise_vec[dof_pos_end_idx:dof_vel_end_idx] = (
            noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
        )
        last_action_end_idx = dof_vel_end_idx + self.num_actions
        obs_noise_vec[dof_vel_end_idx:last_action_end_idx] = 0.0  # previous actions
        if self.cfg.env.num_privileged_obs is not None:
            privileged_extra_obs_noise_vec = torch.zeros(
                self.cfg.env.num_privileged_obs - self.cfg.env.num_propriceptive_obs,
                device=self.device,
            )
        else:
        ...
        if self.cfg.terrain.measure_heights_actor:
            measure_heights_end_idx = last_action_end_idx + len(
                self.cfg.terrain.measured_points_x
            ) * len(self.cfg.terrain.measured_points_y)
            obs_noise_vec[last_action_end_idx:measure_heights_end_idx] = (
    ...
    def _init_buffers(self):
        ...
        # 历史数据观测长度
        self.history_length_dof_pos = self.cfg.observation.history_length_dof_pos
        self.history_length_dof_vel = self.cfg.observation.history_length_dof_vel
        self.history_dof_pos = torch.zeros(
            (self.num_envs, self.num_dof, self.history_length_dof_pos),
            device=self.device,
            dtype=torch.float,
            requires_grad=False,
        )

        self.history_dof_vel = torch.zeros(
            (self.num_envs, self.num_dof, self.history_length_dof_vel),
            device=self.device,
            dtype=torch.float,
            requires_grad=False,
        )
        ...
        self.last_max_feet_height = torch.zeros(
            self.num_envs,
            self.feet_indices.shape[0],
            dtype=torch.float,
            device=self.device,
        ...
        self.current_max_feet_height = torch.zeros(
            self.num_envs,
            self.feet_indices.shape[0],
            dtype=torch.float,
            device=self.device,
    ...
    def _draw_debug_vis(self):
        ...
        # draw height lines
        if (
            not self.terrain.cfg.measure_heights_actor
            and not self.terrain.cfg.measure_heights_critic
        ):
            return
    ...
    def _compute_feet_states(self):
        self.feet_state = self.rigid_body_states[:, self.feet_indices, :]
        self.last_feet_air_time = (
            self.feet_air_time * self.first_contact
            + self.last_feet_air_time * ~self.first_contact
        )
        self.feet_air_time *= ~self.contact_filt
        if self._include_feet_height_rewards:
            self.last_max_feet_height = (
                self.current_max_feet_height * self.first_contact
                + self.last_max_feet_height * ~self.first_contact
            )
            self.current_max_feet_height *= ~self.contact_filt
            self.feet_height = self.feet_state[:, :, 2] - self._get_heights_below_foot()
            self.current_max_feet_height = torch.max(
                self.current_max_feet_height, self.feet_height
            )
        contact = self.contact_forces[:, self.feet_indices, 2] > 1.0
        # Need to filter the contacts because the contact reporting of PhysX is unreliable on meshes
        self.contact_filt = torch.logical_or(contact, self.last_contacts)
        self.last_contacts = contact
        self.first_contact = (self.feet_air_time > 0.0) * self.contact_filt
        self.feet_air_time += self.dt

    # ------------ reward functions----------------
    def _reward_lin_vel_z(self):
        # Penalize z axis base linear velocity
        """作用：惩罚沿 z 轴的线速度，鼓励在水平面上移动。
        实现：计算底座沿 z 轴的线速度的平方，从而抑制垂直方向上的运动。"""
        return torch.square(self.base_lin_vel[:, 2])

    def _reward_ang_vel_xy(self):
        # Penalize xy axes base angular velocity
        """作用：惩罚底座在 x 和 y 轴上的角速度，鼓励绕 z 轴的旋转稳定。
        实现：计算底座在 x 和 y 轴上的角速度平方和，以限制多余的旋转运动。"""
        return torch.sum(torch.square(self.base_ang_vel[:, :2]), dim=1)

    def _reward_orientation(self):
        # Penalize non flat base orientation
        """作用：惩罚底座的不平坦姿态，鼓励机器人保持水平。
        实现：计算投影重力向量在 x 和 y 方向上的平方和，限制底座的倾斜角度。"""
        return torch.sum(torch.square(self.projected_gravity[:, :2]), dim=1)

    def _reward_base_height(self):
        # Penalize base height away from target
        """作用：惩罚底座高度偏离目标值，鼓励保持在特定高度。
        实现：计算底座平均高度与目标高度的差值的平方，确保机器人保持在期望的高度范围内。"""
    ...
    def _reward_torques(self):
        # Penalize torques
        """作用：惩罚关节扭矩的大小，鼓励能量效率。
        实现：计算所有关节扭矩的平方和，限制扭矩过大以降低能耗。"""
        return torch.sum(torch.square(self.torques), dim=1)

    def _reward_dof_vel(self):
        # Penalize dof velocities
        """作用：惩罚关节速度的大小，鼓励关节动作平稳。
        实现：计算所有关节速度的平方和，防止关节过快运动。"""
        return torch.sum(torch.square(self.dof_vel), dim=1)

    def _reward_dof_acc(self):
        # Penalize dof accelerations
        """作用：惩罚关节加速度，鼓励平稳的加速。
        实现：计算关节速度的变化率的平方和，防止加速度过大。"""
        return torch.sum(
    ...
    def _reward_action_rate(self):
        ...
        """作用：惩罚动作的快速变化，鼓励平滑的动作。
        实现：计算当前动作与上一次动作的平方差的和，防止动作突然变化过大，从而提高稳定性。"""
        return torch.sum(torch.square(self.last_actions - self.actions), dim=1)

    def _reward_collision(self):
        # Penalize collisions on selected bodies
        """作用：惩罚选定身体部位的碰撞，鼓励避免碰撞。
        实现：计算接触力大于某阈值的次数，并对其求和。
    ...
    def _reward_termination(self):
        """作用：在终止状态时给出奖励或惩罚，鼓励任务的完成。
        实现：根据重置缓冲区和时间超时缓冲区的状态，计算终止奖励。"""
        # Terminal reward / penalty
        return self.reset_buf * ~self.time_out_buf

    def _reward_dof_pos_limits(self):
        # Penalize dof positions too close to the limit
        """作用：惩罚关节位置过于接近限制，鼓励关节保持在中间位置。
        实现：计算关节位置超过上、下限的偏差，并对偏差求和。"""
    ...
    def _reward_dof_vel_limits(self):
        ...
        """作用：惩罚关节速度过于接近限制，鼓励控制在合理范围内。
        实现：计算关节速度超出限制范围的偏差，并裁剪到最大误差为 1。"""
        return torch.sum(
            (
    ...
    def _reward_torque_limits(self):
        ...
        """作用：惩罚扭矩接近限制值，鼓励合理使用扭矩。
        实现：计算扭矩超过限制范围的偏差，并对偏差求和。"""
        return torch.sum(
            (
    ...
    def _reward_tracking_lin_vel(self):
        ...
        """作用：跟踪线速度命令，鼓励底座沿 x 和 y 轴移动的准确性。
        实现：计算期望和实际线速度之间的平方误差，并使用指数衰减来减少误差对奖励的影响。"""
    ...
    def _reward_tracking_ang_vel(self):
        ...
        """作用：跟踪角速度命令，鼓励绕 z 轴的角速度准确性。
        实现：计算期望与实际角速度之间的平方误差，并使用指数衰减来减少误差对奖励的影响。"""
        ang_vel_error = torch.square(self.commands[:, 2] - self.base_ang_vel[:, 2])
        return torch.exp(-ang_vel_error / self.cfg.rewards.tracking_sigma)

    def _reward_feet_air_time(self):
        # Reward steps between proper duration
        """作用：奖励步伐之间的适当持续时间，避免过短或过长的空中时间。
        实现：计算脚的空中时间在最小和最大范围之外的偏差，并对其进行惩罚。"""
        ...
        rew_airTime_above_max = torch.sum(
            torch.min(
                self.cfg.rewards.max_feet_air_time - self.feet_air_time,
                torch.zeros_like(self.feet_air_time),
            )
            * self.first_contact,
            dim=1,
    ...
    def _reward_stand_still(self):
        ...
        """作用：在零速度命令下惩罚底座的位移和旋转，鼓励静止状态的稳定。
        实现：计算底座在 x、y 轴上的位移和绕 z 轴的旋转，并对其进行惩罚。"""
        reward_lin = torch.abs(self.base_lin_vel[:, :2]) * (self.commands[:, :2] < 0.1)
        reward_ang = (
            torch.abs(self.base_ang_vel[:, -1]) * (self.commands[:, 2] < 0.1)
        ).unsqueeze(dim=-1)
    ...
    def _reward_feet_distance(self):
        ...
        作用：惩罚双脚之间的距离过小，鼓励保持适当的步幅。
        实现：计算每对脚之间的距离，如果距离小于最小允许距离，则进行惩罚。"""
        reward = 0
    ...
    def _reward_survival(self):
        """作用：奖励生存时间，鼓励机器人在环境中存活更长时间。
        实现：对未重置的步数给予奖励，奖励值等于时间步长。"""
        return (~self.reset_buf).float() * self.dt

    def _reward_double_support(self):
        """作用：惩罚双脚同时与地面接触，鼓励单脚支撑的步态。
        实现：检查双脚是否在同一时间点同时接触地面，如果是，则给予惩罚。"""
        # 计算每个脚的接触情况，1 表示接触，0 表示未接触
        left_foot_contact = (
            self.contact_forces[:, self.feet_indices[0], 2] > 0.1
        ).float()
        right_foot_contact = (
            self.contact_forces[:, self.feet_indices[1], 2] > 0.1
        ).float()

        # 检查双脚是否同时接触地面
        double_support = (left_foot_contact * right_foot_contact).float()
